...
@dataclass
class GatingContext:
    """Pre-computed context for parallel hierarchical assignment.

    All data needed to assign a single cluster, avoiding repeated computation.
    """
    hierarchy: Dict[str, List[str]]
    roots: List[str]
    params: Dict[str, Any]
    mset_lookup: Dict[str, Any]  # label -> MarkerSet
    root_marker_stats: Dict[str, Dict[str, Dict[str, float]]]
    # Pre-computed per-cluster data
    cluster_lookups: Dict[str, Dict[str, Dict[str, Any]]] = field(default_factory=dict)
    cluster_labels: Dict[str, set] = field(default_factory=dict)
    cluster_n_cells: Dict[str, int] = field(default_factory=dict)


# Base gating parameters by hierarchy level (tissue-agnostic)
# Tissue-specific rules (root_hard_requirements, root_veto_markers) should be
# provided via the marker map's _gating_params section or explicit config.
BASE_GATING_PARAMS = {
    ...
    # Empty by default - tissue-specific rules loaded from marker map
    "root_hard_requirements": {},
    "root_veto_markers": {},
}

# Backward compatibility: DEFAULT_GATING_PARAMS includes FT-specific rules
# for existing pipelines that don't use the new tissue config system.
# New pipelines should use merge_gating_params() with marker map _gating_params.
...
def merge_gating_params(
    base: Optional[Dict[str, Any]] = None,
    tissue_params: Optional[Dict[str, Any]] = None,
    override: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """Merge gating parameters with priority: base < tissue < override.

    This function enables per-tissue gating rules while maintaining defaults.

    Loading priority (lowest to highest):
    1. base: Base defaults (usually BASE_GATING_PARAMS)
    2. tissue_params: From marker map _gating_params section
    3. override: Explicit config override (CLI or programmatic)

    Parameters
    ----------
    base : dict, optional
        Base gating parameters (default: BASE_GATING_PARAMS)
    tissue_params : dict, optional
        Tissue-specific params from marker map _gating_params
    ...
    Example
    -------
    >>> # Load from marker map
    >>> with open("FT_markers_v9.json") as f:
    ...     marker_map = json.load(f)
    >>> tissue_params = marker_map.get("_gating_params", {})
    >>> params = merge_gating_params(tissue_params=tissue_params)
    """
    import copy

    # Start with base defaults
    result = copy.deepcopy(base or BASE_GATING_PARAMS)

    # Merge tissue-specific params
    if tissue_params:
        for key, value in tissue_params.items():
            if key in result and isinstance(result[key], dict) and isinstance(value, dict):
                # Deep merge for nested dicts (e.g., root_hard_requirements)
...
def log_gating_params(
    params: Dict[str, Any],
    logger: logging.Logger,
    prefix: str = "",
) -> None:
    """Log gating parameters in a readable format.

    Parameters
    ----------
    params : dict
        Gating parameters to log
    logger : logging.Logger
        Logger instance
    prefix : str
        Prefix for log messages (e.g., "[Stage H]" or "[Stage I]")
    """
    if not params:
        logger.info("%s Gating params: None", prefix)
        return

    logger.info("%s Gating params:", prefix)

    # Log level-based thresholds (compact format)
    level_params = ["min_coverage", "min_pos_frac", "min_enrichment", "min_gap", "min_frac_markers_on"]
    for param_name in level_params:
        if param_name in params:
            values = params[param_name]
            if isinstance(values, dict):
                formatted = ", ".join(f"L{k}={v}" for k, v in sorted(values.items(), key=lambda x: int(x[0])))
                logger.info("  %s: {%s}", param_name, formatted)

    # Log scalar params
    scalar_params = ["anti_penalty_hard_gate", "per_cell_gap_threshold", "anti_agg", "root_gap_threshold"]
    for param_name in scalar_params:
        if param_name in params:
            logger.info("  %s: %s", param_name, params[param_name])

    # Log root gating requirements (tissue-specific)
    if "root_hard_requirements" in params:
        reqs = params["root_hard_requirements"]
        if reqs:
            logger.info("  root_hard_requirements:")
            for root, req in reqs.items():
                logger.info(
                    "    %s: marker=%s, min_pos_frac=%.2f, min_enrichment=%.1f",
                    root,
                    req.get("marker", "?"),
                    req.get("min_pos_frac", 0),
                    req.get("min_enrichment", 0),
                )
        else:
            logger.info("  root_hard_requirements: {}")

    if "root_veto_markers" in params:
        vetos = params["root_veto_markers"]
        if vetos:
            logger.info("  root_veto_markers:")
            for root, veto in vetos.items():
                logger.info(
                    "    %s: markers=%s, max_pos_frac=%.2f",
                    root,
                    veto.get("markers", []),
                    veto.get("max_pos_frac", 0),
                )
        else:
            logger.info("  root_veto_markers: {}")


def extract_gating_params_from_marker_map(marker_map: Dict[str, Any]) -> Dict[str, Any]:
    """Extract _gating_params section from marker map if present.

    Parameters
    ----------
    marker_map : dict
        Loaded marker map JSON

    Returns
    -------
    dict
        Gating params dict (empty if not present)

    Example Marker Map Structure
    ----------------------------
    {
        "_marker_map_metadata": {"version": "v9", "tissue": "fallopian_tube"},
        "_gating_params": {
            "root_hard_requirements": {
                "Immune Cells": {"marker": "CD45", "min_pos_frac": 0.30}
            },
            "root_veto_markers": {
                "Immune Cells": {"markers": ["Pan-Cytokeratin"], "max_pos_frac": 0.20}
            }
        },
        "Epithelium": {...},
        "Immune Cells": {...}
    ...
    """
    return marker_map.get("_gating_params", {})
...
def _precompute_marker_stats(
    ...
    OPTIMIZED: Uses pre-computed cluster indices to avoid O(n_clusters × n_cells)
    mask operations. Now O(n_cells) for index building + O(n_clusters × n_markers).
    ...
    obs_clusters = adata.obs[cluster_key].astype(str).values

    # Pre-build cluster -> cell indices mapping (O(n_cells) once)
    from collections import defaultdict
    cluster_indices: Dict[str, np.ndarray] = defaultdict(list)
    for i, c in enumerate(obs_clusters):
        cluster_indices[c].append(i)
    # Convert to numpy arrays for fast indexing
    cluster_indices = {c: np.array(idxs, dtype=np.int64) for c, idxs in cluster_indices.items()}

    if logger:
        logger.debug("Built cluster index mapping for %d clusters in %.2f sec",
                     len(cluster_indices), time.perf_counter() - t0)
    ...
    # Compute per-cluster stats using pre-computed indices
    result: Dict[str, Dict[str, Dict[str, float]]] = {}
    clusters = sorted(cluster_indices.keys())

    for cluster in clusters:
        cell_idxs = cluster_indices[cluster]
        cluster_matrix = matrix[cell_idxs, :]
        result[cluster] = {}
    ...
    if logger:
        logger.debug(
            "Pre-computed stats for %d markers across %d clusters in %.2f sec",
            len(markers),
            len(clusters),
...
def _get_threshold(param_dict: dict, level: int) -> float:
    """Get threshold for a given level from param dict."""
...
def _passes_gate(
    ...
    """Check if a marker set row passes gating criteria.

    Args:
        row: Dict with score, coverage, mean_positive_fraction, etc.
        level: Hierarchy level (0=root, 1=child, etc.)
        cluster_id: Cluster ID for root marker stats lookup
        ctx: GatingContext with params and lookups

    Returns:
        Tuple of (passed, fail_reason)
    """
    label = row.get("label", "")
    mset = ctx.mset_lookup.get(label)
    overrides = mset.gating_overrides if mset and hasattr(mset, 'gating_overrides') and mset.gating_overrides else {}
    params = ctx.params
    ...
    if level == 0 and label in params.get("root_hard_requirements", {}):
        ...
        cluster_stats = ctx.root_marker_stats.get(str(cluster_id), {})
    ...
    min_cov = overrides.get(
        "min_coverage", _get_threshold(params.get("min_coverage", {}), level)
    )
    ...
    min_pos = overrides.get(
        "min_pos_frac", _get_threshold(params.get("min_pos_frac", {}), level)
    )
    min_frac_on = overrides.get(
        "min_frac_markers_on",
        _get_threshold(params.get("min_frac_markers_on", {0: 0.3, 1: 0.25}), level),
    )
    ...
    min_enrich = overrides.get(
        "min_enrichment", _get_threshold(params.get("min_enrichment", {}), level)
    )
    ...
    threshold = params.get("anti_penalty_hard_gate", 1.0)
    if row.get("anti_penalty", 0.0) > threshold:
        return False, "anti_marker_conflict"

    return True, None


def _assign_single_cluster(
    cluster_id: str,
    ctx: GatingContext,
) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    """Assign cell type label to a single cluster (worker function).

    This function is designed to be called in parallel via joblib.
    It does NOT perform per-cell voting (that requires adata access).
    Clusters with ambiguous siblings are marked for post-processing.

    Args:
        cluster_id: Cluster ID to process
        ctx: GatingContext with all pre-computed data

    Returns:
        Tuple of (annotation_dict, list_of_step_dicts)
    """
    label_to_row = ctx.cluster_lookups.get(cluster_id, {})
    cluster_labels = ctx.cluster_labels.get(cluster_id, set())
    cluster_n_cells = ctx.cluster_n_cells.get(cluster_id, 0)

    steps = []

    def get_row(label: str) -> Optional[Dict[str, Any]]:
    ...
    for root in ctx.roots:
        ...
        passed, reason = _passes_gate(row, level=0, cluster_id=cluster_id, ctx=ctx)
        steps.append({
    ...
    if not root_candidates:
        # No root passed - collect fail reasons
        root_fail_reasons = {}
        for step in steps:
            if step["parent_label"] == "ROOT" and step.get("fail_reason"):
    ...
    root_gap_thresh = ctx.params.get("root_gap_threshold", 0.25)
    ...
    # Mark selected root
    for step in steps:
        if step["parent_label"] == "ROOT" and step["child_label"] == current_node:
            step["selected"] = True
    ...
    while current_node in ctx.hierarchy:
        children = ctx.hierarchy[current_node]
        child_candidates = []
        ...
        for child in children:
            ...
            passed, reason = _passes_gate(row, level, cluster_id, ctx)
        ...
        min_gap_threshold = _get_threshold(ctx.params.get("min_gap", {}), level)

        if len(child_candidates) >= 2 and gap < min_gap_threshold:
            # Ambiguous siblings - mark for voting post-processing
            score_winner = child_candidates[0][0]
            runner_up = child_candidates[1][0]
            stop_reason = "ambiguous_siblings"
            needs_voting = True
            ambiguous_candidates = [c[0] for c in child_candidates[:3]]

            # Placeholder composition (will be updated with voting if adata available)
            composition = {
                score_winner: 0.5,
                runner_up: 0.5,
                "_evidence_only": True,
        ...
        # Mark selected child
        for step in steps:
            if step["step_idx"] == step_idx - 1 and step["child_label"] == best_child:
                step["selected"] = True
    ...
    annotation = {
        ...
        "ambiguous_root_candidates": "",
        "root_fail_reasons": "",
        "_needs_voting": needs_voting,
        "_ambiguous_candidates": ambiguous_candidates if needs_voting else [],
    }
...
def assign_labels_hierarchical(
    ...
    Args:
        ...
        n_workers: Number of parallel workers (1=sequential, >1=parallel via joblib)
    ...
    # Pre-compute all cluster lookups (CRITICAL for performance)
    # This replaces the slow iterrows() inside the loop
    logger.info("Pre-computing cluster lookups...")
    grouped_scores = marker_scores.groupby("cluster_id", sort=False)
    n_clusters = grouped_scores.ngroups

    cluster_lookups: Dict[str, Dict[str, Dict[str, Any]]] = {}
    cluster_labels_map: Dict[str, set] = {}
    cluster_n_cells_map: Dict[str, int] = {}

    for cluster_id, cluster_scores in grouped_scores:
        cluster_id_str = str(cluster_id)
        cluster_labels_map[cluster_id_str] = set(cluster_scores["label"].values)

        first_row = cluster_scores.iloc[0] if len(cluster_scores) > 0 else None
        cluster_n_cells_map[cluster_id_str] = int(first_row.get("n_cells", 0)) if first_row is not None else 0

        # Convert to dict of dicts (much faster than iterrows)
        cluster_lookups[cluster_id_str] = {
            row["label"]: row.to_dict()
            for _, row in cluster_scores.iterrows()
        }

    # Build GatingContext with all pre-computed data
    ctx = GatingContext(
        hierarchy=hierarchy,
        roots=roots,
        params=params,
        mset_lookup=mset_lookup,
        root_marker_stats=root_marker_stats,
        cluster_lookups=cluster_lookups,
        cluster_labels=cluster_labels_map,
        cluster_n_cells=cluster_n_cells_map,
    )

    cluster_ids = list(cluster_lookups.keys())
    logger.info("Processing %d clusters...", n_clusters)

    # NOTE: Parallel execution disabled - joblib overhead (~160ms/task) exceeds
    # sequential execution time (~57ms/task) for this workload. The real bottleneck
    # was _precompute_marker_stats which is now optimized. Sequential is faster.
    import time
    t_proc = time.perf_counter()
    results = [_assign_single_cluster(cid, ctx) for cid in cluster_ids]
    logger.info("Processed %d clusters in %.1f sec", n_clusters, time.perf_counter() - t_proc)

    # Unpack results
    annotations = []
    all_steps = []
    clusters_needing_voting = []

    for annotation, steps in results:
        # Check if this cluster needs per-cell voting
        if annotation.get("_needs_voting", False) and adata is not None:
            clusters_needing_voting.append(annotation)
        else:
            # Clean up internal fields
            annotation.pop("_needs_voting", None)
            annotation.pop("_ambiguous_candidates", None)
            annotations.append(annotation)
        all_steps.extend(steps)

    # Post-process clusters that need per-cell voting (sequential, requires adata)
    if clusters_needing_voting:
        logger.info(
            "Running per-cell voting for %d ambiguous clusters...",
            len(clusters_needing_voting)
        )
        for annotation in clusters_needing_voting:
            cluster_id = annotation["cluster_id"]
            candidates = annotation.get("_ambiguous_candidates", [])
            ...
            if candidates:
                ...
                if vote_evidence:
                    # Update composition with voting results
                    old_comp = json.loads(annotation["composition"]) if annotation["composition"] else {}
                    new_comp = vote_evidence["composition"].copy()
                    new_comp["_evidence_only"] = True
                    new_comp["_top_by_score"] = old_comp.get("_top_by_score", "")
                    new_comp["_runner_up_by_score"] = old_comp.get("_runner_up_by_score", "")
                    new_comp["_gap"] = old_comp.get("_gap", 0)
                    new_comp["_threshold"] = old_comp.get("_threshold", 0)
                    new_comp["_cell_margins"] = vote_evidence["margin_stats"]
                    annotation["composition"] = json.dumps(new_comp)
